...
from services.doctor_service import get_doctor_by_id, get_doctors, get_doctors_by_category
...
def get_all_doctors(
    ...
    return [_build_doctor_out(doctor, base_url) for doctor in doctors]
...
def get_doctor(
    ...
    base_url = str(request.base_url).rstrip("/")
    return _build_doctor_out(doctor, base_url)


@router.get("/category/{category_id}", response_model=list[DoctorOut])
def get_doctors_in_category(
    category_id: int,
    request: Request,
    db: Session = Depends(get_db),
):
    doctors = get_doctors_by_category(db, category_id)
    base_url = str(request.base_url).rstrip("/")
    return [_build_doctor_out(doctor, base_url) for doctor in doctors]