...
def create_review(db: Session, user_id: int, data: DoctorReviewCreate) -> DoctorReview:
    ...
    existing = db.query(DoctorReview).filter(DoctorReview.user_id == user_id, DoctorReview.doctor_id == data.doctor_id).first()
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="You have already rated this doctor")

    review = DoctorReview(user_id=user_id, doctor_id=data.doctor_id, rating=data.rating, created_at=datetime.now(timezone.utc))
    db.add(review)
