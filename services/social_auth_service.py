...
def google_login(db: Session, token: str) -> dict:
    try:
        google_user = id_token.verify_oauth2_token(token, google_requests.Request(), settings.GOOGLE_CLIENT_ID)
    except ValueError:
    ...
    if not google_id or not email:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid Google account information")
    ...
    user = db.query(User).filter(User.google_id == google_id, User.is_deleted == False).first()
    ...
    access_token = create_access_token({"sub": str(user.user_id), "type": "user"})
